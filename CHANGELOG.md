...
## [0.11.0-pilot] - Pilot 发布（真实中介试用版）

> Batch 01-04 全部验收通过后的阶段性版本（commit 9d81aae 起步，本版本含 Batch 04 收尾）。
> 外部评审建议：本版本作为 Pilot/MVP，交由 1-2 个真实中介试用一周，收集真实反馈
> 后再启动 Batch 05（SaaS 化）。反馈台账与观察清单见 `docs/pilot/`。

- 功能面：Navy+Slate+Warm Paper 设计系统 / 教员推荐优先首页 / 登录三入口分离 /
  B 端经营工作台 + 订单工作区（双栏）/ AI 批量录单四步工作流（分诊+字段级校对）/
  投递审核响应式 / 合规页 / 可观测性 / 审计面
- 已知边界（如实记录）：B 端投递无推荐解释数据（API 缺口）、财务无按单查询参数、
  成交时间戳缺失、AI 无字段级数值置信度、363 处存量 slate 待按业务域迁移
- Pilot 期间 Batch 05（SaaS 化）暂缓——规格书已存档 `docs/batches/batch-05-saas-spec.md`，
  开工前提：Pilot 反馈收集完成

**基线：pytest 223 passed / ruff 0 / mypy 0 / vitest 59 / build 绿**
