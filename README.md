...
> 以下截图由真实微信群里采集的家教订单文本驱动：走线上同款 AI 解析管道灌入演示库，非摆拍数据。
...
2. **教员**打开中介专属橱窗（邀请码链接即获客落地页），在地图上找单，系统按科目、年级、距离（常驻地）、院校标签、历史成交智能推荐；
3. 投递 → 候选 → **线下收定金** → 试课 → 尾款 → 成交，每一步都有状态机守卫与资金流水；
4. 成交后中介给教员打分，**信用画像**回流到推荐排序，一键生成可转发的**教员公开成绩单**，形成正向筛选。

## 三端角色

| 角色 | 入口 | 核心能力 |
|---|---|---|
| **教员**（C 端 H5） | `/teacher/board/{邀请码}` | 手机号+密码登录；地图找单、个性化推荐、投递/取消、试课后解锁家长联系方式、通知、我的费用、收到的评价、公开成绩单、资料与简历库自助维护 |
| **中介**（B 端后台） | `/admin/login` | AI 批量导入订单、投递审核全流程、订单/地图管理、财务流水（凭证留档/筛选/导出）、教员管理与拉黑、站内通知 |
| **平台老板** | 教员登录页 → 老板入口 | 中介/邀请码管理（一次性初始密码、重置）、教员全局封禁、经营看板（漏斗/排行/收入） |
...
- **资金台账**（线下收款模式）：定金 → 尾款 → 退款/没收，每笔登记操作人与订单，支持转账凭证留档，净收入公式保证台账守恒；退款封顶实收、零退款强制补记没收。
- **信用体系**：成交数 / 违约数 / 评价均分批量聚合，进入投递列表与推荐排序；中介级黑名单 + 平台级封禁双层拦截；一单一评沉淀为公开成绩单。
- **通知与触达**：教员（投递流转/评价）+ 中介（新投递/取消投递/订单临期）双通道站内通知，未读角标；站外触达管道（企业微信/webhook）可插拔扩展。
...
地图与 AI 解析需在 `.env` / `frontend/.env.development` 配置 DeepSeek Key、高德 Key；未配置时地图降级、AI 解析不可用，其余功能照常。
...
MySQL 8.4 + Redis 7 + API + Nginx 等多容器编排，HTTPS 建议走 Cloudflare 或 Caddy（操作单见 docs）。
部署后可跑 `python scripts/smoke_test.py --base https://your-domain` 一条命令全链路自检（29 项断言）。

前端生产构建需配置 `frontend/.env.production`（样例见 [.env.production.example](frontend/.env.production.example)），
其中 `VITE_DEFAULT_INVITE_CODE` 指向平台自有橱窗——无码访问根路径的流量将落在该橱窗上。

## 测试与 CI

```bash
python -m pytest tests/ -q   # 212 个用例：认证/资金守卫/状态机/通知/评价/看板/限流/简历/资料编辑/分页契约
...
- **隐私**：C 端与公开橱窗坐标降精度至小区级；订单原文手机号/微信号/QQ 脱敏；家长联系方式仅试课后解锁；公开成绩单不暴露联系方式
- **失效**：中介停用/教员封禁实时回查；改密后已签发 JWT 立即作废（`token_valid_after`）
- **财务**：金额 Decimal 半进一舍入；退款封顶实收；零退款强制补记没收流水；每笔流水登记操作人角色，凭证随流水保留
...
docs/images/   # README 界面截图（真实解析数据驱动）
```

## 路线图

- [ ] 高德 JSAPI 专用 Key + 安全密钥（当前兼容态见 `frontend/.env.development.local` 说明）
- [ ] 教员侧列表（我的投递/通知/费用）分页加载
- [ ] 投递接口 query 传参改请求体（RESTful）
- [ ] 短信触达通道（企业微信/webhook 已就绪）
